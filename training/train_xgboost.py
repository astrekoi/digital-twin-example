"""XGBoost regressor for forecasting telemetry time series - multi-horizon.

EXTERNAL MACHINE ONLY. Train on Windows/Linux, deploy artifacts to Pi.

Direct multi-output: train one XGBRegressor per horizon, save as
``joblib.dump({h: model_h}, "<id>.joblib")``.
...
- After search: refit on (train+val) using best_params (one regressor per
  horizon for the multi-output artifact).
- Final metrics computed ONCE on the held-out test partition.
- Trials log written to ``data/training_logs/<model_id>_optuna.csv``.
- ``meta.training.optuna`` records search_space, best_params,
  best_val_rmse_at_15 and the trials_log_path.

GPU: ``device="cuda"`` if a tiny smoke fits, else ``device="cpu"``.
"""
...
XGB_SEARCH_SPACE: dict[str, list] = {
...
def _build_xgb(params: dict, *, device: str):
...
def train(
    ...
    if tune:
        ...
        # Legacy 80/20 path: keep behaviour stable for the v1 pipeline.
        train_df, test_df = time_split(features_df, test_size=test_size)
        val_df = train_df.iloc[0:0]  # empty placeholder
    ...
    device = device_pick_xgb()
    logger.info(
        "xgboost: device=%s, %d features, %d horizons, train=%d val=%d test=%d, tune=%s",
        device, len(feature_cols), len(horizons_min),
        len(train_df), len(val_df), len(test_df), tune,
    )

    optuna_meta: dict | None = None
    best_params = dict(XGB_DEFAULTS)
    ...
    if tune:
        ...
        best_params_search, best_val_rmse, trials_df, search_space_actual = run_optuna_search(
            lambda params: _build_xgb(params, device=device),
            XGB_SEARCH_SPACE,
            X_train=X_train,
            y_train=y_train_h15,
            X_val=X_val,
    ...
    # Refit on (train+val) with best_params; one regressor per horizon for the
    # multi-output artifact. Test partition stays untouched until the line that
    # builds y_pred a few statements below.
    ...
    for h, tcol in zip(horizons_min, target_cols, strict=True):
        ...
        m = _build_xgb(best_params, device=device)
        m.fit(X_fit, y_fit)
        models[h] = m
    ...
    logger.info(
        "TEST: xgboost trained in %.2fs; rmse@15=%.4f rmse@60=%.4f device=%s tuned=%s",
        duration,
    ...
    write_meta_json(
        ...
        test_size=test_size,
        notes=(
            f"XGBoost multi-horizon (one regressor per horizon). "
            f"params={best_params}, device={device}, tree_method=hist."
            + notes_extra
        ),
    ...
    print(
        f"OK xgboost {model_id}: train(+val)={len(fit_df)} test={len(test_df)} "
        f"duration={duration:.2f}s device={device} tuned={tune} "
        f"test_rmse@15={metrics['rmse_per_horizon'].get('15', float('nan')):.4f}"
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="XGBoost trainer (multi-horizon, Optuna-tunable)")
    ...
    parser.add_argument("--tune", action="store_true", help="Run Optuna search; otherwise legacy 80/20 with defaults.")
    ...
    # Backwards-compat: if NOT tuning, keep legacy 0.2 test-size (matches v1 runs).
